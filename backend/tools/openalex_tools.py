...
def get_citation_details(search_term: str) -> str:
    ...
    if not openalex_id: return f"Error: Paper matching '{search_term}' not found or has no OpenAlex ID."
    citation_data = _openalex_api_call(f"/{openalex_id}/citations", filters={'citation_format': 'bibtex'})
    citation = citation_data.get('citation', 'Citation not available.') if citation_data else "Citation not available."
...
def get_cited_works(search_term: str, limit: int = 5) -> CitedWorksList:
    ...
    for work_data in results['results']:
        authors = ", ".join([a['author']['display_name'] for a in work_data.get('authorships', [])])
