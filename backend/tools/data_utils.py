...
def load_paper_data():
    ...
    df["openalex_id_short"] = df["openalex_id"].str.replace(
...
def _get_openalex_id_from_title(search_term: str) -> Optional[str]:
    ...
    match = PAPER_DF[PAPER_DF["title"].str.contains(search_term, case=False, na=False)]
    return match.iloc[0]["openalex_id_short"] if not match.empty else None


def _openalex_api_call(
    endpoint_suffix: str, filters: Dict[str, str] = {}
